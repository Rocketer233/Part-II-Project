...
def mha2gqa(state_dict, groups_idx, num_heads, transpose_layer=True):
    ...
    for layer_id in range(num_layers):
        layer_name = f'model.decoder.layers.{layer_id}.self_attn.k_proj.weight' # name of the attention layer projection matrices
        layer = state_dict[layer_name]
        if transpose_layer: layer = layer.transpose(0, 1)
        ...
        layer_name = f'model.decoder.layers.{layer_id}.self_attn.k_proj.bias' # name of the attention layer projection matrices
        layer = state_dict[layer_name]
        # print(layer.shape)
    ...
    for layer_id in range(num_layers):
        layer_name = f'model.decoder.layers.{layer_id}.self_attn.v_proj.weight' # name of the attention layer projection matrices
        layer = state_dict[layer_name]
        if transpose_layer: layer = layer.transpose(0, 1)
        # Code below unit tested
        layer = torch.stack(torch.tensor_split(layer, num_heads, dim=1), dim=0)
        ...
        layer_name = f'model.decoder.layers.{layer_id}.self_attn.v_proj.bias' # name of the attention layer projection matrices
        layer = state_dict[layer_name]
        # print(layer.shape)
        layer = torch.stack(torch.tensor_split(layer, num_heads))
        # print(layer.shape)
